...
const SUMMARY_FILE = path.join(
  OUTPUT_DIR,
  'historical_backfill_summary.csv'
);
...
if (selectedSymbols.length === 0) {
  throw new Error(
    'No symbols selected for historical intraday backfill.'
  );
}
...
async function main() {
  ...
  log(
    'START',
    `Intraday backfilling ${selectedSymbols.length} symbols: `
      + selectedSymbols
  ...
  try {
    for (
      const [index, item]
      of selectedSymbols.entries()
    ) {
      log(
        'FETCH',
        `[${index + 1}/${selectedSymbols.length}] `
          + item.symbol
      );
      ...
      if (normalizedBars.length === 0) {
        const error =
          'No valid intraday bars returned';
