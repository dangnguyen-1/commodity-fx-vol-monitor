...
const SUMMARY_FILE = path.join(OUTPUT_DIR, 'historical_daily_backfill_summary.csv');
...
async function main() {
  ...
  log('START', `Daily backfilling ${allSymbols.length} symbols`);
  ...
  for (const [index, item] of allSymbols.entries()) {
    ...
    log(
      'SAVED',
      `${item.symbol} timeframe=${TIMEFRAME} bars=${result.bars.length} earliest=${new Date(oldest.time * 1000).toISOString()}`
    );
