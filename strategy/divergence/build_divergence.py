...
def add_expected_response_model(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Raw underreaction:
    # Positive means FX moved less than expected in the model's expected direction
    # for positive expected returns.
    # Negative means FX moved more than expected or moved opposite.
    df["fx_underreaction_1d"] = (
        df["expected_fx_response_1d"] - df["actual_fx_response_1d"]
    )

    # Directional underreaction:
    # Positive means FX has underreacted in the same direction as our signal.
    # Negative means FX may have already moved too much or moved against the setup.
    df["directional_fx_underreaction_1d"] = (
        df["signal_direction"] * df["fx_underreaction_1d"]
    )

    # Scale by FX volatility so divergence is comparable across pairs.
    ...
    df["directional_fx_underreaction_vol_scaled"] = (
        df["signal_direction"] * df["fx_underreaction_vol_scaled"]
    )
...
def add_divergence_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["expected_response_score"] = bounded_score(
        df["expected_fx_response_vol_scaled"],
